...
class Node:
    def __init__(self, label, weight=1.0):
        ...
        self.name = label       # Will be overwritten with the dynamic prefix
        self.weight = weight
        self.parent = None
        self.children = []
    ...
    def __repr__(self):
        return f"Node({self.name}, w={self.weight:.2f}, children={len(self.children)})"
...
# ---------------------------
# Basic Node and Hierarchy Classes
# ---------------------------

class Node:
    def __init__(self, label, weight=0.0, parent=None):
        self.label = label
        self.weight = weight
        self.parent = parent
        self.children = []
        self.prefix = ""       # the dynamic prefix (e.g., O, A, A1)
        self.index = -1        # position in the final 1D ordering
...
class HierarchyObject:
    def __init__(self):
        self.origin = None            # Label of the root node (e.g., "Origin")
        self.graph = {}               # dictionary: node_label -> {child_label: weight, ...}
        self.nodes = {}               # dictionary: node_label -> Node object
        self.ordered_nodes = []       # final ordered list of Node objects
        self.hpc_log = []             # log of HPC cost or iterations (if needed)
        self.json_cache = {}          # JSON cache for LLM calls or simulated responses

    def load_from_json(self):
        """Loads JSON cache if available."""
        try:
            with open("cache.json", "r") as f:
                self.json_cache = json.load(f)
            logging.info("Loaded JSON cache.")
        except FileNotFoundError:
            logging.info("No JSON cache found; starting fresh.")
    
    def save_to_json(self):
        """Saves the JSON cache to file."""
        with open("cache.json", "w") as f:
            json.dump(self.json_cache, f, indent=4)
    
    def assign_initial_indices(self):
        """Assigns 1D indices to nodes based on current ordering."""
        for idx, node in enumerate(self.ordered_nodes):
            node.index = idx

    def set_ordered_nodes(self, ordered_labels):
        """Sets the ordered_nodes list based on a list of node labels."""
        self.ordered_nodes = [self.nodes[label] for label in ordered_labels]
        self.assign_initial_indices()

    def add_node(self, label, weight=0.0, parent_label=None):
        """Adds a node to the hierarchy and updates the graph structure."""
        if label in self.nodes:
            return self.nodes[label]
        node = Node(label, weight, parent_label)
        self.nodes[label] = node        
        # Update the parent's children & the graph structure:
        if parent_label:
            if parent_label in self.nodes:
                self.nodes[parent_label].add_child(node)
            self.graph.setdefault(parent_label, {})[label] = weight
        return node
...
def initialize_problem_space(use_json=True):
    """
    Initialize the hierarchical problem space.
    
    This function builds a dummy hierarchy for demonstration:
      - The origin ("Origin") and some top-level categories (e.g. "A", "B", "C")
      - Each top-level has subcategories (e.g. "A1", "A2", "A3", etc.)
    """
    hierarchy = HierarchyObject()
    if use_json:
        hierarchy.load_from_json()
    
    # Create the origin
    origin = hierarchy.add_node("Origin")
    hierarchy.origin = "Origin"
    
    # Create top-level nodes (children of Origin) with causal weights
    for label, weight in [("A", 0.9), ("B", 0.8), ("C", 0.7)]:
        hierarchy.add_node(label, weight, "Origin")
    
    # Create subcategories for each top-level node
    for parent_label in ["A", "B", "C"]:
        for i, sub in enumerate([f"{parent_label}1", f"{parent_label}2", f"{parent_label}3"]):
            hierarchy.add_node(sub, 0.75 - i*0.05, parent_label)
    
    # Assign dynamic prefixes; Origin is "O", top-level labels as they are, subcategories remain unchanged.
    hierarchy.nodes["Origin"].prefix = "O"
    for label, node in hierarchy.nodes.items():
        if label != "Origin":
            # In this demo, assume top-level labels have length 1.
            if len(label) == 1:
                node.prefix = label
            else:
                node.prefix = label  # e.g., "A1" remains "A1"
    return hierarchy

def linearizeOneD(graph, root):
    """
    Given a graph (dictionary mapping node label -> {child_label: weight, ...})
    and a root label, return a list of node labels in a linear 1D ordering.
    
    The ordering rule:
      1) Start with the root.
      2) Append the root's children sorted by descending weight.
      3) Iterate the order and append children of each node (sorted by descending weight).
    """
    order = [root]
    
    # Sort and append root's children
    if root in graph:
        sorted_children = sorted(graph[root].keys(), key=lambda x: graph[root][x], reverse=True)
        order.extend(sorted_children)
    
    # Walk through the order and add children
    i = 1
    while i < len(order):
        current = order[i]
        if current in graph:
            sorted_children = sorted(graph[current].keys(), key=lambda x: graph[current][x], reverse=True)
            order.extend(sorted_children)
        i += 1
    return order
...
def plot_hierarchy(hierarchy):
    """
    Plot a dummy adjacency matrix for the ordered nodes.
    
    The raw node labels are used as tick labels, and we overlay dynamic prefixes on the diagonal.
    """
    n = len(hierarchy.ordered_nodes)
    # For demonstration, create an identity matrix as a placeholder
    matrix = np.eye(n)
    fig, ax = plt.subplots(figsize=(10, 8))
    cax = ax.imshow(matrix, cmap='viridis', interpolation='none')
    fig.colorbar(cax)
    
    # Use raw labels as tick labels:
    labels = [node.label for node in hierarchy.ordered_nodes]
    ax.set_xticks(np.arange(n))
    ax.set_xticklabels(labels, rotation=45, fontsize=10)
    ax.set_yticks(np.arange(n))
    ax.set_yticklabels(labels, fontsize=10)
    ax.set_title("Hierarchy Adjacency Matrix")
    return fig, ax
...
def get_cached_llm_response(prompt):
    ...
    In production, this would call the LLM API and cache responses (or use --use_mock).
    Here we simply simulate a response.
    """
    logging.info("Simulated LLM query for prompt: " + prompt)
    return "simulated_response"

def main5():
    # In our shell script, if the --use_mock flag is passed, we would set use_json=True.
    use_json = True  # For demonstration, always use JSON cache here.
    
    # Step 1: Initialize the problem space.
    hierarchy = initialize_problem_space(use_json)
    
    # For debugging: print the generated graph structure.
    print("Graph structure:", hierarchy.graph)
    
    # Step 2: Linearize the hierarchy to get a 1D ordering.
    ordered_labels = linearizeOneD(hierarchy.graph, hierarchy.origin)
    print("Ordered labels:", ordered_labels)
    
    # Update the hierarchy with the final ordered list of nodes.
    hierarchy.set_ordered_nodes(ordered_labels)
    for node in hierarchy.ordered_nodes:
        print(f"Node: {node.label}, Prefix: {node.prefix}, Index: {node.index}")
    
    # Step 3: Plot the main adjacency matrix.
    fig, ax = plot_hierarchy(hierarchy)
    
    # Step 4: Overlay the dynamic prefixes on the diagonal.
    for i, node in enumerate(hierarchy.ordered_nodes):
        ax.text(i, i, node.prefix,
    ...
    # (Optional) Save the JSON cache to simulate HPC/LLM logging.
    hierarchy.save_to_json()
# Set up logging.
logging.basicConfig(level=logging.INFO)

#def setup_custom_font():
    # Use the locally installed custom font.
    #plt.rcParams['font.family'] = 'Noto Color Emoji'  # Replace with your custom font name
...
class Node:
    def __init__(self, label, weight=0.0, parent_label=None):
        self.label = label
        self.weight = weight
        self.parent_label = parent_label
        self.children = []
        self.index = None
        self.prefix = None  # To be set later via dynamic prefixing
...
class HierarchyObject:
    def __init__(self):
        self.nodes = {}             # Mapping: label -> Node instance
        self.ordered_nodes = []     # Final 1D ordering of nodes
        self.graph = {}             # Structure: {parent_label: {child_label: weight, ...}}
        self.origin = None          # Typically "Origin"
        self.llm_cache = {}         # Will be loaded from or saved to llm_cache.json

    def load_from_json(self, filename="llm_cache.json"):
        try:
            with open(filename, "r") as f:
                self.llm_cache = json.load(f)
            logging.info("Loaded LLM cache from " + filename)
        except Exception as e:
            logging.info("LLM cache load failed: " + str(e))
            self.llm_cache = {}

    def save_to_json(self, filename="llm_cache.json"):
        with open(filename, "w") as f:
            json.dump(self.llm_cache, f, indent=4)
        logging.info("Saved LLM cache to " + filename)
    ...
    def set_ordered_nodes(self, ordered_labels):
        self.ordered_nodes = [self.nodes[label] for label in ordered_labels if label in self.nodes]
        self.assign_initial_indices()
...
def linearizeOneD(graph, root):
    """
    Given a graph (dict: node -> {child: weight, ...}) and a root,
    returns a list of node labels in a linear 1D ordering.
    """
    order = [root]
    if root in graph:
        sorted_children = sorted(graph[root].keys(), key=lambda x: graph[root][x], reverse=True)
    ...
    while i < len(order):
        current = order[i]
        if current in graph:
            kids = sorted(graph[current].keys(), key=lambda x: graph[current][x], reverse=True)
            order.extend(kids)
        i += 1
    return order

def apply_dynamic_prefixes(raw_labels):
    """
    Computes dynamic prefixes for each label.
    - The origin gets 'O'
    - Top-level nodes receive sequential letters (A, B, C, ...)
    - Subcategory nodes get a composite prefix (e.g., A1, A2, ...)
    
    Returns:
       final_prefixes (list of str), top_mapping (dict)
    """
    if not raw_labels:
        return [], {}
    
    final_prefixes = []
    # First label is origin.
    final_prefixes.append("O")
    
    groups = []  # Each element: (top_level_label, [sub_label, ...])
    current_top = None
    ...
    top_mapping = {}
    for i, (raw_top, _) in enumerate(groups):
        top_mapping[raw_top] = alphabet[i]
    
    for raw_top, sub_list in groups:
        prefix_top = top_mapping[raw_top]
        final_prefixes.append(prefix_top)
        for j, _ in enumerate(sub_list):
            final_prefixes.append(f"{prefix_top}{j+1}")
    
    return final_prefixes, top_mapping

def plot_matrix(matrix, labels):
    """Plots the matrix with the raw labels as tick labels."""
    #setup_custom_font()
    fig, ax = plt.subplots(figsize=(10, 8))
    cax = ax.imshow(matrix, cmap='viridis', interpolation='none')
    fig.colorbar(cax)
    ...
    ax.set_yticks(np.arange(n))
    ax.set_yticklabels(labels, fontsize=10)
    ax.set_title("Hierarchical Causal Matrix")
    return fig, ax

def overlay_dynamic_prefixes(ax, hierarchy):
    """
    Overlays the dynamic prefixes on the diagonal.
    As each diagonal cell represents a link from a node to itself,
    the prefix is doubled (e.g., "A" becomes "A|A").
    """
    for i, node in enumerate(hierarchy.ordered_nodes):
        double_prefix = f"{node.prefix}|{node.prefix}"
        ax.text(i, i, double_prefix,
                color='red', fontsize=12, fontweight='bold',
                ha='center', va='center', zorder=10)
        logging.info(f"Overlaying dynamic prefix '{double_prefix}' at position ({i},{i})")

# =========================
# Pipeline: Build & Refine the Hierarchy
# =========================

def initialize_problem_space(use_json=True):
    """
    Initializes the problem space by building a HierarchyObject.
    Loads from JSON (llm_cache.json) if available.
    Creates the origin, top-level categories, and subcategories.
    Applies linearization and dynamic prefixing.
    """
    hierarchy = HierarchyObject()
    if use_json:
        hierarchy.load_from_json()  # Load LLM cache if available

    # Create origin.
    origin = hierarchy.add_node("Origin")
    hierarchy.origin = "Origin"

    # Create top-level nodes (for demonstration, labels "A", "B", "C" used as names).
    for label, weight in [("A", 0.9), ("B", 0.8), ("C", 0.7)]:
        hierarchy.add_node(label, weight, "Origin")
    ...
    # Generate a raw ordering using the current graph structure.
    raw_labels = linearizeOneD(hierarchy.graph, hierarchy.origin)
    final_prefixes, top_mapping = apply_dynamic_prefixes(raw_labels)

    # Set dynamic prefixes on each node.
    for label, prefix in zip(raw_labels, final_prefixes):
        if label in hierarchy.nodes:
            hierarchy.nodes[label].prefix = prefix

    # Update the 1D ordered list in the hierarchy.
    hierarchy.set_ordered_nodes(raw_labels)

    # Debug printing.
    print("Ordered Labels:", raw_labels)
    for node in hierarchy.ordered_nodes:
        print(f"Node: {node.label}, Prefix: {node.prefix}, Index: {node.index}")

    return hierarchy

def build_matrix(hierarchy):
    """
    Builds (or loads) the adjacency matrix.
    Here we simulate by creating an identity matrix of size = number of nodes.
    In your full implementation, this would be replaced by the actual causal matrix.
    """
    n = len(hierarchy.ordered_nodes)
    matrix = np.eye(n)
    return matrix

def final_plot(hierarchy):
    """
    Uses the current hierarchy object to build the matrix and plot it.
    Node labels (i.e. prefixes) are used as tick labels.
    """
    matrix = build_matrix(hierarchy)
    # Use the node label (already set to dynamic prefix) for the axis ticks.
    x_labels = [node.label for node in hierarchy.ordered_nodes]
    fig, ax = plot_matrix(matrix, x_labels)
    overlay_dynamic_prefixes(ax, hierarchy)
    plt.tight_layout()
    plt.show()
...
def final_pipeline():
    """
    Central pipeline:
      1. Initializes the hierarchy.
      2. Computes dynamic prefixes.
      3. Updates each node's label to be its prefix.
      4. Sets the ordered nodes.
      5. Generates one final plot.
      6. Saves JSON cache (simulating HPC/LLM caching).
    """
    # Initialize your hierarchy; if use_json==True, load from JSON cache.
    hierarchy = initialize_problem_space(use_json=True)
    
    # Build a raw ordering for nodes (using a linearization method such as linearizeOneD)
    raw_labels = linearizeOneD(hierarchy.graph, hierarchy.origin)
    
    # Compute dynamic prefixes (returns a list of prefix strings and a top-level mapping)
    final_prefixes, top_mapping = apply_dynamic_prefixes(raw_labels)
    
    # Update the nodes in the hierarchy:
    # (Assumes hierarchy.nodes is a dict of {raw_label: Node} and each Node has attributes 'label' and 'prefix'.)
    for label, prefix in zip(raw_labels, final_prefixes):
        if label in hierarchy.nodes:
            hierarchy.nodes[label].prefix = prefix
            # Set the node's label to be its prefix (as required)
            hierarchy.nodes[label].label = prefix
    
    # Set the ordered list of nodes in the hierarchy (for plotting/submatrix lookup)
    hierarchy.set_ordered_nodes(raw_labels)
    
    # Debug prints to verify the ordering and prefixes:
    print("Ordered Node Labels (Prefixes):")
    for node in hierarchy.ordered_nodes:
        print(f"Node: {node.label}, Prefix: {node.prefix}, Index: {node.index}")
    
    # Generate the single final plot.
    final_plot(hierarchy)
    
    # Save any JSON cache (for HPC/LLM calls) from the hierarchy.
    hierarchy.save_to_json()

def main():
    """
    The single main execution point:
      1. Runs the final_pipeline which builds and refines the hierarchy.
      2. Produces one final plot.
    """
    try:
        final_pipeline()
    except Exception as e:
        logging.error(f"[Error] An error occurred in the main pipeline: {e}")
        raise

if __name__ == "__main__":
    main()

def linearize_one_d(graph, root):
    """
    Constructs the final 1D ordering of nodes from a hierarchical graph,
    beginning at the root node. The graph is assumed to be a nested dict where:
       graph[node] -> {child: weight, ...}
    
    Steps:
      1. Initialize the ordering with the root.
      2. Append the root's children (sorted by descending weight) to the ordering.
      3. Walk through the ordering from index 1 onward: for each node, if it has
         children in the graph, sort those by descending weight and append any
         child that is not already in the ordering.
    
    Args:
        graph (dict): A dictionary mapping nodes to a dict of children and their weights.
        root (str): The starting node (e.g., "Origin").
    
    Returns:
        list: The computed 1D ordering of nodes.
    """
    # 1) Initialize the final ordering with the root.
    order = [root]
    
    # 2) Get the root's children sorted by descending weight (if any) and append.
    if root in graph:
        root_children = sorted(graph[root].keys(), key=lambda child: graph[root][child], reverse=True)
        order.extend(root_children)
    
    # 3) Iteratively walk through the order and append each node's children.
    i = 1  # Start after the root.
    while i < len(order):
        current_node = order[i]
        if current_node in graph:
            children = sorted(graph[current_node].keys(), key=lambda child: graph[current_node][child], reverse=True)
...
# Minimal testing of the function:
if __name__ == '__main__':
    # Construct an example graph based on our hierarchy:
    # "Origin" has children A, B, C; then A has A1, A2, A3; etc.
    graph = {
    ...
    final_order = linearize_one_d(graph, "Origin")
    print("Final 1D Order:", final_order)
...
def query_llm_for_importance(prompt):
    """
    Dummy LLM call.
    Replace this with your actual LLM API call.
    For now, returns a random number to simulate a weight.
    """
    logging.info(f"Calling LLM with prompt: {prompt}")
    return np.random.rand()

def get_causal_weight(child, parent, origin, use_mock=False):
    """
    Get the causal weight linking parent -> child in the context of origin.
    Uses a JSON cache to avoid duplicate LLM calls.
    """
    prompt = (f"Evaluate the causal importance: How important is '{child}' "
              f"relative to its parent '{parent}' in the context of the origin '{origin}'? "
              "Provide a weight between 0 (lowest) and 1 (highest).")
    key = f"{origin}:{parent}:{child}"
    ...
    if key in cache:
        weight = cache[key]
        logging.info(f"Using cached weight for key '{key}': {weight}")
    else:
        # If use_mock is True, use a fixed mock value.
        if use_mock:
            weight = 0.5
            logging.info(f"[MOCK] Using default weight for '{key}': {weight}")
        else:
            weight = query_llm_for_importance(prompt)
        # Save the new weight to the cache.
        cache[key] = weight
        with open(cache_file, "w") as f:
            json.dump(cache, f, indent=4)
        logging.info(f"Saved new weight for key '{key}': {weight}")
    return weight

def linearizeOneD(graph, root, origin, use_mock=False):
    """
    Linearizes the hierarchy represented by 'graph' starting from 'root'.
    Uses get_causal_weight() to evaluate links that are missing weights.
    
    graph: dict mapping node -> {child: weight or None}
    root: string (starting node), in your case this would be "Origin"
    origin: context for LLM queries
    use_mock: if True, bypasses actual LLM calls.
    """
    order = [root]
    
    # If the root has children, sort them by weight.
    if root in graph:
        children = list(graph[root].keys())
        children_weighted = []
        for child in children:
            # If weight is missing (None), request a causal weight.
            if graph[root][child] is None:
                w = get_causal_weight(child, root, origin, use_mock)
                # Optionally update the graph with the newly retrieved weight.
                graph[root][child] = w
            else:
                w = graph[root][child]
            children_weighted.append((child, w))
        
        # Sort children descending by weight.
        children_weighted.sort(key=lambda x: x[1], reverse=True)
        sorted_children = [child for child, _ in children_weighted]
        order.extend(sorted_children)
    
    # Walk through the current order to append each child node's children.
    i = 1
    while i < len(order):
        current_node = order[i]
        if current_node in graph:
            kids = list(graph[current_node].keys())
            kids_weighted = []
            for kid in kids:
                if graph[current_node][kid] is None:
                    w = get_causal_weight(kid, current_node, origin, use_mock)
                    graph[current_node][kid] = w
                else:
                    w = graph[current_node][kid]
                kids_weighted.append((kid, w))
            kids_weighted.sort(key=lambda x: x[1], reverse=True)
            sorted_kids = [kid for kid, _ in kids_weighted]
            order.extend(sorted_kids)
        i += 1
...
def build_identity_matrix(size):
    """
    For demonstration, build a simple identity matrix.
    In your full system, this could be an adjacency/similarity matrix.
    """
    return np.eye(size)

def plot_matrix(matrix, labels):
    """
    Placeholder function to plot the matrix.
    (In your full code, implement plotting with dynamic overlays on the diagonal.)
    """
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(figsize=(10, 8))
    ...
    plt.title("Adjacency Matrix with Causal Hierarchy")
    plt.tight_layout()
    plt.show()
...
def main():
    """
    Main execution:
    1. Builds a sample graph structure representing your hierarchy.
    2. Uses linearizeOneD() to build a 1D ordering via LLM-derived causal weights.
    3. Constructs a demonstration matrix.
    4. (Later) This matrix will be refined and plotted with overlays.
    """
    # Example hierarchical graph: keys are nodes; values are dicts mapping children -> weight.
    # For now, weights are set to None to simulate missing values that require LLM calls.
    graph = {
        "Origin": {"A": None, "B": None, "C": None},
    ...
    origin = "Origin"
    # Check if "--use_mock" flag is present in command-line arguments.
    use_mock = "--use_mock" in sys.argv

    logging.info("Starting hierarchy linearization...")
    ordered_nodes = linearizeOneD(graph, origin, origin, use_mock)
    logging.info("Final 1D ordering:")
    for idx, node in enumerate(ordered_nodes):
        logging.info(f"Index {idx}: {node}")
    
    # For demonstration, build a simple matrix (identity matrix here).
    size = len(ordered_nodes)
    matrix = build_identity_matrix(size)
    
    # Plot the matrix with raw labels.
    plot_matrix(matrix, ordered_nodes)
